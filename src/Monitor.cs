...
namespace arr_scanner
{
    class Monitor
    {
        ...
        private void ForceImport()
        {
            if (Settings.Provider() == Settings.NAME_4KRADARR) { return;} // not implemented yet to radarr
        ...
        private void ForceImport()
        {
            if (Settings.Provider() == Settings.NAME_3DRADARR) { return;} // not implemented yet to radarr
            ...
            private void Scan()
        {
            dynamic dyn = new ExpandoObject();
            string apiUrl = "";
            if (Settings.Provider() == Settings.NAME_4KSONARR)
            {
                dyn.name = "missingEpisodeSearch";
                apiUrl = $"/api/v3/command?apikey={Settings.APIKey}";
            }
